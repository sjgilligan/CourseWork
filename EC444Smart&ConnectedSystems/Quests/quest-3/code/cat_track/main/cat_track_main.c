...
extern float highly_active_time;
extern float inactive_time;

extern float inactive_time_d;
extern float active_time_d;
extern float highly_active_time_d;

extern activity_t activity;
extern char payload[100];
extern uint8_t cat_rank;
extern uint8_t display_rank;

extern cat_activity_t cat;


static void log_data()
{
    while(1){
        char activity_char[15] = "inactive";
        ...
        // TIME, TEMP, ACTIVITY
        char cat_rank_c[20];
        char display_rank_c[20];
        sprintf(cat_rank_c,"  CAT RANK %d ", cat_rank);
        sprintf(display_rank_c,"  CAT RANK %d ", display_rank);
		
		int i = 0;
		while(display_rank_c[i] != '\0'){
			glo_user_buf[i] = getBinaryValue(display_rank_c[i]);
			i++;
		}
    glo_display_count = i;
        sprintf(payload,"%f %f %f %f\n", cat.total_inactive_time, cat.total_active_time, cat.total_highly_active_time, temperature);
        printf("Payload: %s\n", payload);
        printf("cat rank: %s\n", cat_rank_c);
        printf("display rank: %d\n" , display_rank);
...
void app_main() {
  ...
  xTaskCreate(test_adxl343,"test_adxl343", 4096, NULL,  configMAX_PRIORITIES-9, NULL);
  ...
  xTaskCreate(buzzer_task, "buzzer_task",4096, configMAX_PRIORITIES-10,5, NULL );
  xTaskCreate(udpate_activity, "udpate_activity", 4096, configMAX_PRIORITIES-11, 5, NULL);
